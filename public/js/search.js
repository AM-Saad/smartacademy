...
const formHandler = async (e) => {
    ...
    const data = await fetchdata(`/public/search?q=${userInput}`, 'get', {}, true)
    if (data) {
        $('.search_container .loading').addClass('none')

        const teachers = data.json.teachers
        // const filteredWords = words.filter(word => word.toLowerCase().includes(userInput)).sort().splice(0, 5)
        dropEl.innerHTML = ''
        teachers.forEach(item => {
            const listEl = document.createElement('li')
            const listLink = document.createElement('a')
            listLink.setAttribute('href', `/teachers/${item._id}`)
            listLink.textContent = item.name + ' - ' + item.subject
        ...
        let totalChildrenHeight = dropEl.children[0].offsetHeight * teachers.length
